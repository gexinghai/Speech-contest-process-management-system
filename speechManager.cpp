...
void SpeechManager::speechContest()
{
	...
	for (vector<int>::iterator it = v_Src.begin(); it != v_Src.end(); ++it)
	{
		...
		//将打分数据放入临时小组容器中
		groupScore.insert(make_pair(avg, *it));//得分 编号
		//每6人取前三名
		if (num % 6 == 0)
		{
...
void SpeechManager::loadRecord()
{
	...
	while (ifs >> data)
	{
		...
		while (true)
		{
			...
			string tmp = data.substr(start, pos - start);//找到,分割，参数1：起始位置，参数2：截取长度
			//cout << tmp << endl;
			v.push_back(tmp);
			start = pos + 1;
		}
...
void SpeechManager::clearRecord()
{
	...
	if (select == 1)
	{
		ofstream ofs("speech.csv", ios::trunc);//trunc如果存在删除重建
		ofs.close();
